...
<?php
	...
	if(isset($_COOKIE['session_user_id'])){
		$session_user_id = mysqli_real_escape_string($db, $_COOKIE['session_user_id']);
		$session_token   = mysqli_real_escape_string($db, $_COOKIE['session_token']);
		// check for matching username and sessionkey
		$query = "select * from users where id='$session_user_id' AND session_token='$session_token';";
	...
	function user_is_admin(){
		...
		$is_admin = isset($db_permissions['is_admin']);
